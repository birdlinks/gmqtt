...
func genClient() *Client {
	c, _ := net.Pipe()
	return NewClient(c, circ.NewReader(128, 8), circ.NewWriter(128, 8), new(system.Info), receiveMaximum)
}
...
func TestNewClient(t *testing.T) {
	...
	require.NotNil(t, cl)
	require.NotNil(t, cl.Inflight.GetAll())
...
func BenchmarkNewClient(b *testing.B) {
	c, _ := net.Pipe()
	for n := 0; n < b.N; n++ {
		NewClient(c, circ.NewReader(16, 4), circ.NewWriter(16, 4), nil, receiveMaximum)
	}
}

func TestNewClientStub(t *testing.T) {
	cl := NewClientStub(nil, receiveMaximum)
...
func BenchmarkNewClientStub(b *testing.B) {
	for n := 0; n < b.N; n++ {
		NewClientStub(nil, receiveMaximum)
	}
}
...
func TestClientNoteSubscription(t *testing.T) {
	...
	require.Equal(t, packets.SubOptions{QoS: 0}, cl.Subscriptions["a/b/c"])
}
...
func TestClientWritePacket(t *testing.T) {
	for i, tt := range pkTable {
		r, w := net.Pipe()
		cl := NewClient(r, circ.NewReader(128, 8), circ.NewWriter(128, 8), new(system.Info), 256)
		cl.Start()
...
func TestClientWritePacketInvalidPacket(t *testing.T) {
	c, _ := net.Pipe()
	cl := NewClient(c, circ.NewReader(16, 4), circ.NewWriter(16, 4), new(system.Info), receiveMaximum)
	cl.Start()
...
func TestInflightSet(t *testing.T) {
	...
	require.NotNil(t, cl.Inflight.GetAll()[1])
	require.NotEqual(t, 0, cl.Inflight.GetAll()[1].Sent)
...
func TestInflightDelete(t *testing.T) {
	...
	require.NotNil(t, cl.Inflight.GetAll()[3])

	q := cl.Inflight.Delete(3)
	require.Equal(t, true, q)
	im := cl.Inflight.GetAll()[3]
	require.Equal(t, (*InflightMessage)(nil), im)
