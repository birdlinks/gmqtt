...
import (
	...
	"github.com/birdlinks/gmqtt/internal/listeners/auth"
	"github.com/birdlinks/gmqtt/internal/persistence"
	"github.com/birdlinks/gmqtt/internal/pkg/circ"
	"github.com/birdlinks/gmqtt/internal/pkg/clients"
	"github.com/birdlinks/gmqtt/internal/pkg/packets"
	"github.com/birdlinks/gmqtt/internal/pkg/topics"
...
func setupClient() (s *Server, cl *clients.Client, r net.Conn, w net.Conn) {
	...
	r, w = net.Pipe()
	cl = clients.NewClient(w, circ.NewReader(256, 8), circ.NewWriter(256, 8), s.System, s.Options.ReceiveMaximum)
	cl.ID = "mochi"
...
func TestServerEstablishConnectionOKCleanSession(t *testing.T) {
	...
	c, _ := net.Pipe()
	cl := clients.NewClient(c, circ.NewReader(256, 8), circ.NewWriter(256, 8), s.System, s.Options.ReceiveMaximum)
	cl.ID = "mochi"
...
func TestServerEstablishConnectionPromptSendLWT(t *testing.T) {
	...
	go func() {
		w.Write([]byte{
			...
			'M', 'Q', 'T', 'T', // Protocol Name
			4,     // Protocol Version
			0,     // Packet Flags - clean session 0 or 1
			0, 45, // Keepalive
			0, 5, // Client ID - MSB+LSB
			'm', 'o', 'c', 'h', 'i', // Client ID,
		})
...
func TestServerProcessSubscribe(t *testing.T) {
	...
	err := s.processPacket(cl, packets.Packet{
		...
		//Qoss:     []byte{0, 1},
		SubOss: []packets.SubOptions{{QoS: 0}, {QoS: 1}},
	})
	require.NoError(t, err)
	time.Sleep(10 * time.Millisecond)
	w.Close()
	...
	require.Equal(t, byte(0), cl.Subscriptions["a/b/c"].QoS)
	require.Equal(t, byte(1), cl.Subscriptions["d/e/f"].QoS)
...
func TestServerProcessUnsubscribe(t *testing.T) {
	...
	cl.NoteSubscription("a/b/c", packets.SubOptions{QoS: 0})
	cl.NoteSubscription("d/e/f", packets.SubOptions{QoS: 1})
	cl.NoteSubscription("a/b/+", packets.SubOptions{QoS: 2})
...
func TestServerLoadSubscriptions(t *testing.T) {
	...
	cl := clients.NewClientStub(s.System, s.Options.ReceiveMaximum)
	cl.ID = "test"
	s.Clients.Add(cl)
...
func TestServerLoadInflight(t *testing.T) {
	...
	w, _ := net.Pipe()
	defer w.Close()
	c1 := clients.NewClient(w, nil, nil, nil, s.Options.ReceiveMaximum)
	c1.ID = "client1"
	s.Clients.Add(c1)
