...
func TestWriteRetrieveDeleteSubscription(t *testing.T) {
	...
	require.Equal(t, "", subs[0].T)
	//require.Equal(t, "a/b/c", subs[0].Filter)
...
func TestWriteRetrieveDeleteInflight(t *testing.T) {
	...
	require.Equal(t, "", msgs[0].T)
	require.Equal(t, "a/b/c", msgs[0].TopicName)
